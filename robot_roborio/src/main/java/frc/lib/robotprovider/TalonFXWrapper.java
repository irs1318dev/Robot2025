...
public class TalonFXWrapper implements ITalonFX
{
    ...
    private StatusSignal<Double> output;
    ...
    private void internalSet(TalonFXControlMode mode, int slotId, double value, double feedForward)
    {
        switch (mode)
        {
            ...
            case MotionMagicPositionExpo:
                if (this.useVoltageCompensation)
                {
                    MotionMagicExpoVoltage mmevRequest;
                    if (this.currentControlRequest instanceof MotionMagicExpoVoltage)
                    {
                        mmevRequest = (MotionMagicExpoVoltage)this.currentControlRequest;
                        mmevRequest.withPosition(value);
                    }
                    else
                    {
                        mmevRequest = new MotionMagicExpoVoltage(value);
                    }

                    mmevRequest.withFeedForward(feedForward);
                    mmevRequest.withSlot(slotId);
                    this.wrappedObject.setControl(mmevRequest);
                }
                else
                {
                    MotionMagicVelocityDutyCycle mmvdcRequest;
                    if (this.currentControlRequest instanceof MotionMagicVelocityDutyCycle)
                    {
                        mmvdcRequest = (MotionMagicVelocityDutyCycle)this.currentControlRequest;
                        mmvdcRequest.withVelocity(value);
                    }
                    else
                    {
                        mmvdcRequest = new MotionMagicVelocityDutyCycle(value);
                    }

                    mmvdcRequest.withFeedForward(feedForward);
                    mmvdcRequest.withSlot(slotId);
                    this.wrappedObject.setControl(mmvdcRequest);
                }

                break;

            case MotionMagicVelocity:
                if (this.useVoltageCompensation)
                {
                    MotionMagicVelocityVoltage mmvvRequest;
                    if (this.currentControlRequest instanceof MotionMagicVelocityVoltage)
                    {
                        mmvvRequest = (MotionMagicVelocityVoltage)this.currentControlRequest;
                        mmvvRequest.withVelocity(value);
                    }
                    else
                    {
                        mmvvRequest = new MotionMagicVelocityVoltage(value);
                    }

                    mmvvRequest.withFeedForward(feedForward);
                    mmvvRequest.withSlot(slotId);
                    this.wrappedObject.setControl(mmvvRequest);
                }
                else
                {
                    MotionMagicVelocityDutyCycle mmvdcRequest;
    ...
    public void setOutputUpdateRate(double frequencyHz)
    {
        if (this.output == null)
        {
            this.output = this.wrappedObject.getDutyCycle();
        }

        CTREStatusCodeHelper.printError(
            this.output.setUpdateFrequency(frequencyHz, TalonFXWrapper.timeoutSecs),
            this.talonId,
            "TalonFX.setOutputUpdateRate-Position");
    }
    ...
    public void setMotionMagicExpoPIDVS(double p, double i, double d, double v, double s, double cruiseVelocity, double velocityVoltage, double accelerationVoltage, int slotId)
    {
        ...
        MotionMagicConfigs motionMagicConfigs = new MotionMagicConfigs();
        motionMagicConfigs.MotionMagicCruiseVelocity = cruiseVelocity;
        motionMagicConfigs.MotionMagicExpo_kV = velocityVoltage;
        motionMagicConfigs.MotionMagicExpo_kA = accelerationVoltage;
        CTREStatusCodeHelper.printError(
            this.currentConfigurator.apply(motionMagicConfigs, TalonFXWrapper.timeoutSecs),
            this.talonId,
            "TalonFXWrapper.setMotionMagicPIDVS-MM");
    ...
    public void setCurrentLimit(boolean enabled, double currentLimit, double triggerThresholdCurrent, double triggerThresholdTime, boolean statorLimiting, double statorCurrentLimit)
    {
        ...
        currentLimitsConfigs.SupplyCurrentLowerLimit = triggerThresholdCurrent;
        currentLimitsConfigs.SupplyCurrentLowerTime = triggerThresholdTime;
    ...
    public double getPosition()
    {
        ...
        return this.position.getValue().magnitude();
    }
    ...
    public double getVelocity()
    {
        ...
        return this.velocity.getValue().magnitude();
    }
