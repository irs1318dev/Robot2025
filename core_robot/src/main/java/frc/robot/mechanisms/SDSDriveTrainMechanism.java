...
{
    private static enum SlewRateLimitingMode
    {
        Normal,
        Medium,
        High
    ...
    private SlewRateLimitingMode currentSlewRateLimitingMode;
        ...
        ITimer timer)
    {
        ...
        if (TuningConstants.SDSDRIVETRAIN_USE_TRANSLATIONAL_RATE_LIMITING)
        {
            this.xVelocityLimiter = new SlewRateLimiter(
                this.timer,
                TuningConstants.SDSDRIVETRAIN_TRANSLATIONAL_VELOCITY_MAX_NEGATIVE_RATE_DEFAULT,
                TuningConstants.SDSDRIVETRAIN_TRANSLATIONAL_VELOCITY_MAX_POSITIVE_RATE_DEFAULT,
                0.0);
        ...
        this.currentSlewRateLimitingMode = SlewRateLimitingMode.Normal;
    ...
    {
        if (TuningConstants.SDSDRIVETRAIN_READSENSORS_ONLY_IN_TEST_MODE && mode == RobotMode.Test)
        {
            return;
        }
        ...
        if (TuningConstants.SDSDRIVETRAIN_USE_TRANSLATIONAL_RATE_LIMITING_LEVELS)
        {
            SlewRateLimitingMode newSlewRateLimitingMode;
            if (this.elevator.getPosition() >= TuningConstants.SDSDRIVETRAIN_TRANSLATE_LIMIT_ELEVATOR_THRESHOLD_HIGH)
            {
                newSlewRateLimitingMode = SlewRateLimitingMode.High;
            }
            else if (this.elevator.getPosition() >= TuningConstants.SDSDRIVETRAIN_TRANSLATE_LIMIT_ELEVATOR_THRESHOLD_MEDIUM)
            {
                newSlewRateLimitingMode = SlewRateLimitingMode.Medium;
            }
            else
            {
                newSlewRateLimitingMode = SlewRateLimitingMode.Normal;
            }

            if (this.currentSlewRateLimitingMode != newSlewRateLimitingMode)
            {
                this.currentSlewRateLimitingMode = newSlewRateLimitingMode;
                switch (newSlewRateLimitingMode)
                {
                    case High:
                        this.xVelocityLimiter.configureRate(
                            TuningConstants.SDSDRIVETRAIN_TRANSLATIONAL_VELOCITY_MAX_NEGATIVE_RATE_HIGH,
                            TuningConstants.SDSDRIVETRAIN_TRANSLATIONAL_VELOCITY_MAX_POSITIVE_RATE_HIGH);
                        this.yVelocityLimiter.configureRate(
                            TuningConstants.SDSDRIVETRAIN_TRANSLATIONAL_VELOCITY_MAX_NEGATIVE_RATE_HIGH,
                            TuningConstants.SDSDRIVETRAIN_TRANSLATIONAL_VELOCITY_MAX_POSITIVE_RATE_HIGH);
                        break;
                    case Medium:
                        this.xVelocityLimiter.configureRate(
                            TuningConstants.SDSDRIVETRAIN_TRANSLATIONAL_VELOCITY_MAX_NEGATIVE_RATE_MEDIUM,
                            TuningConstants.SDSDRIVETRAIN_TRANSLATIONAL_VELOCITY_MAX_POSITIVE_RATE_MEDIUM);
                        this.yVelocityLimiter.configureRate(
                            TuningConstants.SDSDRIVETRAIN_TRANSLATIONAL_VELOCITY_MAX_NEGATIVE_RATE_MEDIUM,
                            TuningConstants.SDSDRIVETRAIN_TRANSLATIONAL_VELOCITY_MAX_POSITIVE_RATE_MEDIUM);
                        break;
        ...
        else if (this.driver.getDigital(DigitalOperation.DriveTrainEnableMaintainDirectionMode))
        {
    ...
    private void calculateSetpoints(boolean useFieldOriented, boolean maintainOrientation)
    {
        ...
        if (maintainPositionMode || this.driver.getDigital(DigitalOperation.DriveTrainSteerMode))
        {
            for (int i = 0; i < SDSDriveTrainMechanism.NUM_MODULES; i++)
            {
                ...
                AnglePair anglePair = Helpers.getClosestAngle(moduleSteerPositionGoal, currentAngle, true);
        ...
        if (this.driver.getDigital(DigitalOperation.DriveTrainPathMode))
        {
            ...
            if (useFieldOriented)
            {
                ...
                AnglePair anglePair = Helpers.getClosestAngle(angleGoal, this.robotYaw, false);
        ...
        else
        {
            ...
            else if (useFieldOriented)
            {
                ...
                if (yawGoal != TuningConstants.MAGIC_NULL_VALUE)
                {
                    ...
                    AnglePair anglePair = Helpers.getClosestAngle(yawGoal + yawAdj, this.robotYaw, false);
                    this.desiredYaw = anglePair.getAngle();
                }
